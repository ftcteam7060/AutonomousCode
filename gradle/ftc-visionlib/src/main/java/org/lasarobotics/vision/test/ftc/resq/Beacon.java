...
public final class Beacon {
    ...
    private Size beaconSizeFinal = new Size();
    public Double matchedEllipses (Mat gray) {
        ...
        //ellipses.get(0).isInside(BeaconScoring.ScoredContour.getList(scoredContoursRed).get(0));
        return matchEllipses(ellipses);
    }
    public BeaconAnalysis analyzeColor(List<Contour> contoursRed, List<Contour> contoursBlue, Mat img, Mat gray) {
        ...
        List<Ellipse> ellipses = ellipseLocationResult.getEllipses();
        //ellipses.get(0).isInside(BeaconScoring.ScoredContour.getList(scoredContoursRed).get(0));
        matchEllipses(ellipses);
//        double widthI;
//        double widthJ;
//        for (int i = 0; i < ellipses.size()-1; i++) {
//            for (int j = i; j < ellipses.size()-1; j++) {
//                widthI = ellipses.get(i).width();
//                widthJ = ellipses.get(j).width();
//                if (Math.abs(widthI - widthJ) <= 10){
//
//                }
//            }
//        }
        ...
        if (scoredEllipses.size() > 0) {
            ...
            Drawing.drawText(img, Integer.toString(scoredEllipses.size())
                    ,  new Point(0, 8), 1.0f, new ColorGRAY(255), Drawing.Anchor.BOTTOMLEFT);

        }
        ellipseCount = scoredEllipses.size();
        ...
        beaconSizeFinal = new Size(beaconActualWidth * scale, beaconActualHeight * scale);
        ...
        //Get the center point of the beacon
//        beaconCenter.x = beaconTopLeft.x + (beaconBottomRight.x - beaconTopLeft.x) / 2;
//        beaconCenter.y = beaconTopLeft.y + (beaconBottomRight.y - beaconTopLeft.y) / 2;
    ...
    public Point getBeaconCenter(){
        return this.beaconCenter;
    }
    public int getEllipseCount() {
        return this.ellipseCount;
    }

    public Size getBeaconSize() {
        return this.beaconSizeFinal;
    }

    public Double matchEllipses (List<Ellipse> ellipses) {
        double widthI;
        double widthJ;
        for (int i = 0; i < ellipses.size()-1; i++) {
            for (int j = i; j < ellipses.size()-1; j++) {
